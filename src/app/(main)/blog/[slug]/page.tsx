...
function renderMarkdown(content: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      elements.push(
        <h2 key={i} className="m-display" style={{ fontSize: 22, margin: '32px 0 12px', color: 'var(--ink)' }}>
          {line.slice(3)}
        </h2>
      );
    } else if (line.startsWith('### ')) {
      elements.push(
        <h3 key={i} className="m-display" style={{ fontSize: 17, margin: '24px 0 10px', color: 'var(--ink)' }}>
          {line.slice(4)}
        </h3>
      );
    } else if (line.startsWith('| ')) {
        ...
        <div key={`table-${i}`} style={{ overflowX: 'auto', margin: '16px 0' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            <thead>
              <tr>
                {headers.map((h, hi) => (
                  <th key={hi} style={{ padding: '10px 14px', background: 'var(--bg-2)', border: '1px solid var(--line)', textAlign: 'left', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13 }}>
                    {h.trim()}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.map((row, ri) => (
                <tr key={ri}>
                  {row.split('|').filter(s => s.trim()).map((cell, ci) => (
                    <td key={ci} style={{ padding: '10px 14px', border: '1px solid var(--line-soft)', color: 'var(--ink-2)', fontSize: 13.5 }}>
                      {cell.trim()}
                    </td>
                  ))}
                </tr>
      ...
      elements.push(
        <ul key={`ul-${i}`} style={{ margin: '12px 0', paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      elements.push(
        <ol key={`ol-${i}`} style={{ margin: '12px 0', paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {listItems.map((item, li) => (
            <li key={li} style={{ color: 'var(--ink-2)', fontSize: 15, lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: renderInline(item) }} />
          ))}
        </ol>
      ...
      elements.push(
        <p key={i} style={{ margin: '12px 0', color: 'var(--ink-2)', lineHeight: 1.7, fontSize: 15 }}
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const related = BLOG_POSTS.filter(p => p.slug !== post.slug && (p.category === post.category || p.tags.some(t => post.tags.includes(t)))).slice(0, 3);
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 48, alignItems: 'start' }}>
          ...
          <article>
            ...
            <div style={{
              ...
              height: 240, fontSize: 96, marginBottom: 32,
            ...
            <h1 className="m-display" style={{ fontSize: 'clamp(24px, 3.5vw, 36px)', margin: '0 0 16px', lineHeight: 1.15 }}>
              {post.title}
            </h1>

            <p style={{ fontSize: 16, color: 'var(--ink-2)', lineHeight: 1.6, marginBottom: 32, paddingBottom: 32, borderBottom: '1px solid var(--line-soft)' }}>
              {post.excerpt}
            </p>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 40, paddingTop: 28, borderTop: '1px solid var(--line-soft)' }}>
              ...
              {post.tags.map(tag => (
                <span key={tag} className="m-chip" style={{ height: 30, fontSize: 12 }}>#{tag}</span>
              ))}
            </div>

            {/* CTA */}
            <div className="m-surface-2" style={{ padding: '28px 32px', marginTop: 36, borderRadius: 'var(--radius-l)' }}>
              <h3 className="m-display" style={{ fontSize: 20, margin: '0 0 10px' }}>
                İkinci el ekipman arıyor musun?
              </h3>
              <p style={{ color: 'var(--ink-2)', fontSize: 14.5, marginBottom: 20 }}>
                Motorya'da binlerce ikinci el motosiklet ekipmanı seni bekliyor. Güvenli ödeme, kargo takibi.
              </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {related.map(p => (
                  <Link key={p.slug} href={`/blog/${p.slug}`} style={{ textDecoration: 'none', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <div>
                      <p style={{ margin: 0, fontSize: 13.5, fontFamily: 'var(--font-display)', fontWeight: 600, lineHeight: 1.3, color: 'var(--ink)' }}>
                        {p.title}
                      </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {['Güvenlik', 'Ekipman', 'Bakım', 'Rota & Seyahat', 'Ürün İnceleme', 'Hukuk & Sigorta'].map(cat => (
                  <Link key={cat} href="/blog" style={{ fontSize: 14, color: 'var(--ink-2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--line-soft)', textDecoration: 'none' }}>
                    {cat}
                    <ChevronLeft size={14} style={{ transform: 'rotate(180deg)', color: 'var(--ink-3)' }} />
                  </Link>
                ))}
              </div>
            </div>
